...
class BaseAgent:
    ...
    def _needs_escalation(self, content: str) -> bool:
        """只识别显式升级标记，避免回复中提到“人工客服”就误触发工单。"""
        return "[ESCALATE]" in content


class GeneralAgent(BaseAgent):
    agent_type    = AgentType.GENERAL
    system_prompt = (
        "你是 SakikoMind 智能客服。友好、简洁地回答用户问题。"
...
class AgentOrchestrator:
    ...
    async def run(self, req: Request) -> OrchestratorResult:
        ...
        """
        t0 = time.monotonic()
        logger.info(f"[trace_id={req.request_id}] 编排开始")
        ...
        logger.info(
            f"[trace_id={req.request_id}] 意图完成 intent={req.intent.value if req.intent else 'other'} "
            f"urgency={req.urgency.name if req.urgency else 'UNKNOWN'}"
        )
        ...
        if len(collaboration) > 1:
            logger.info(
                f"[trace_id={req.request_id}] 并行路由 agents="
                f"{','.join(agent_type.value for agent_type in collaboration)}"
            )
            return await self.run_parallel(req, collaboration)

        # 2. 路由：选择 Agent 类型
        agent_type = collaboration[0] if collaboration else self._route(req.intent, req.urgency)
        logger.info(f"[trace_id={req.request_id}] 路由完成 agent={agent_type.value}")
        ...
        if response.escalate or req.urgency == UrgencyLevel.CRITICAL or req.intent == IntentCategory.ESCALATION:
            escalated = True
            logger.warning(f"[trace_id={req.request_id}] 触发升级 urgency={req.urgency}")
            # 生产环境：此处创建工单、通知人工客服

        latency_ms = (time.monotonic() - t0) * 1000
        logger.info(
            f"[trace_id={req.request_id}] 编排完成 agent={response.agent_type.value} "
            f"escalated={escalated} duration_ms={latency_ms:.1f}"
        )
    ...
    async def run_parallel(self, req: Request, agent_types: List[AgentType]) -> OrchestratorResult:
        ...
        latency_ms = (time.monotonic() - t0) * 1000
        logger.info(
            f"[trace_id={req.request_id}] 并行编排完成 responses={len(parts)} "
            f"escalated={escalated} duration_ms={latency_ms:.1f}"
        )
        return OrchestratorResult(
            request_id=req.request_id,
    ...
    def _route(self, intent: Optional[IntentCategory], urgency: Optional[UrgencyLevel]) -> AgentType:
        ...
        if urgency == UrgencyLevel.CRITICAL:
            if intent == IntentCategory.TECHNICAL and self._pool.get(AgentType.TECHNICAL):
    ...
    def _collaboration_targets(self, req: Request) -> List[AgentType]:
        ...
        if req.intent == IntentCategory.BILLING or any(kw in msg for kw in billing_kws):
    ...
    async def _execute(self, req: Request, agent_type: AgentType) -> AgentResponse:
        ...
        if not response.success and agent_type != AgentType.GENERAL:
            logger.warning(f"[trace_id={req.request_id}] {agent_type.value} 失败，降级到 GeneralAgent")

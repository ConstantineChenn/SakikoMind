...
只返回 JSON，例如: {{"relevance": 0.9, "accuracy": 0.8, "completeness": 0.7, "helpfulness": 0.85}}"""
    ...
    async def judge(
        ...
        for attempt in range(1, self.MAX_ATTEMPTS + 1):
            try:
                ...
                logger.warning(f"LLM Judge 第 {attempt} 次失败: {ex}")
    ...
    @staticmethod
    def _parse_scores(raw: str) -> QualityScores:
        """从模型输出中提取并校验四项评分，拒绝空响应和不完整 JSON。"""
        if not raw or not raw.strip():
            raise ValueError("模型未返回可解析正文")
        ...
        if any(field not in data for field in fields):
            raise ValueError("评分 JSON 缺少必要字段")
        scores = {field: float(data[field]) for field in fields}
        if any(score < 0.0 or score > 1.0 for score in scores.values()):
            raise ValueError("评分必须位于 0.0 到 1.0 之间")
...
class EndToEndEvaluator:
    ...
    async def run(
        ...
        if intent_cases:
            ...
            results.append(EvalResult(
                ...
                metadata={
                    ...
                    "failed_cases": intent_metrics.get("failed_cases", []),
                },
            ))
        ...
        if dialog_cases:
            for i, case in enumerate(dialog_cases):
                ...
                for r in case_results:
                    if r.metadata.get("judge_failed", False):
                        continue
                    for k in all_scores:
                        if k in r.scores:
        ...
        inconclusive = sum(
            ...
            if result.metadata.get("judge_failed", False)
        )
        evaluated_results = [
            result for result in results
            if not result.metadata.get("judge_failed", False)
        ]
        passed_count = sum(1 for result in evaluated_results if result.passed)
        pass_rate = passed_count / len(evaluated_results) if evaluated_results else 0.0
    ...
    @property
    def last_snapshot_path(self) -> Optional[str]:
        """最近一次按时间戳保存的评测快照路径。"""
    ...
    def _save_baseline(self, report: EvalReport) -> None:
        ...
        try:
            ...
            self._baseline_path.write_text(report_json, encoding="utf-8")
            snapshot_dir = self._baseline_path.parent / "snapshots"
            snapshot_dir.mkdir(parents=True, exist_ok=True)
            timestamp = datetime.now().strftime("%Y%m%dT%H%M%S%f")
            snapshot_path = snapshot_dir / f"eval-{timestamp}.json"
            snapshot_path.write_text(report_json, encoding="utf-8")
...
_FALLBACK_INTENT_CASES: List[IntentTestCase] = [
...
_FALLBACK_DIALOG_CASES: List[Dict[str, Any]] = [
...
_FIXED_CASES_PATH = pathlib.Path(__file__).parent.parent / "data" / "eval" / "fixed_cases.json"


def load_fixed_eval_cases(path: Optional[pathlib.Path] = None) -> tuple[List[IntentTestCase], List[Dict[str, Any]]]:
    """读取版本化固定评测集；文件异常时保留内置回退样本。"""
    cases_path = path or _FIXED_CASES_PATH
    try:
        ...
        intent_cases = [
            ...
            for item in raw["intent_cases"]
        ]
        dialog_cases = list(raw.get("dialog_cases", []))
        if not intent_cases:
            raise ValueError("intent_cases 不能为空")
        return intent_cases, dialog_cases
    except Exception as ex:
        logger.warning(f"读取固定评测集失败，使用内置回退样本: {ex}")
        return _FALLBACK_INTENT_CASES, _FALLBACK_DIALOG_CASES


DEFAULT_INTENT_CASES, DEFAULT_DIALOG_CASES = load_fixed_eval_cases()
